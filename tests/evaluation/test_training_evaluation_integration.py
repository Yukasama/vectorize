...
class TestTrainingEvaluationIntegration:
    ...
    def test_training_followed_by_evaluation(self, client: TestClient) -> None:
        """Test complete workflow: train model, then evaluate it."""
        ...
        # Step 3: Start evaluation while training (should work independently)
        evaluation_payload = {
            "model_tag": MINILM_MODEL_TAG,
        ...
        # Step 5: Verify both tasks have different IDs
        assert training_task_id != evaluation_task_id

        cleanup_trained_models()

    def test_multiple_datasets_training_and_evaluation(self, client: TestClient) -> None:
        """Test training with multiple datasets and evaluating each."""
        ensure_minilm_model_available()

        # Step 1: Training with multiple datasets
        training_payload = {
            "model_tag": MINILM_MODEL_TAG,
        ...
        # Step 2: Evaluate first dataset
        eval_payload_1 = {
            "model_tag": MINILM_MODEL_TAG,
            "dataset_id": DATASET_ID_1,
        }

        eval_response_1 = client.post("/evaluation/evaluate", json=eval_payload_1)
        assert eval_response_1.status_code == HTTP_202_ACCEPTED
        eval_task_id_1 = extract_task_id(eval_response_1, "evaluation")

        # Step 3: Evaluate second dataset
        eval_payload_2 = {
            "model_tag": MINILM_MODEL_TAG,
            "dataset_id": DATASET_ID_2,
        }

        eval_response_2 = client.post("/evaluation/evaluate", json=eval_payload_2)
        assert eval_response_2.status_code == HTTP_202_ACCEPTED
        eval_task_id_2 = extract_task_id(eval_response_2, "evaluation")

        # Step 4: Verify all task IDs are different
        task_ids = {training_task_id, eval_task_id_1, eval_task_id_2}
        assert len(task_ids) == 3, "All task IDs should be unique"

        # Step 5: Check all tasks have valid status
        for task_id, endpoint in [
            (training_task_id, "training"),
            (eval_task_id_1, "evaluation"),
            (eval_task_id_2, "evaluation"),
        ]:
