"""Dataset service for file processing and classification."""
...
from txt2vec.datasets.constants import DatasetType, FileFormat
...
class DatasetService:
    """Service for dataset operations including file handling and classification."""

    async def upload_file(
        self, file: UploadFile, sheet_name: int = 0
    ) -> dict[str, Any]:
    ...
    def _process_upload(
        ...
        """
        try:
            df: Final = self._load_dataframe(file_path, file_format, sheet_name)

            self._validate_dataframe(df)
            dataset_type: Final = self._classify_dataset(df)

            csv_filename: Final = self._generate_unique_filename(original_filename)
            self._save_dataframe(df, csv_filename)
    ...
    @staticmethod
    def _classify_dataset(df: pd.DataFrame) -> DatasetType:
    ...
    def _load_csv(self, file_path: str) -> pd.DataFrame:
        ...
        """
        delimiter: Final = self._detect_delimiter(file_path)
        ...
        for encoding in ["utf-8", "latin1", "cp1252"]:
            try:
                ...
                logger.debug(
                    "CSV parsing with {} encoding failed: {}", encoding, str(e)
                )
                continue
