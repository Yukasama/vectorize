...
class VRAMEviction:
    ...
    def __init__(self, usage_tracker: UsageTracker, vram_monitor: VRAMMonitor) -> None:
        ...
        self.model_vram_sizes: dict[str, int] = {}  # model_tag -> vram_bytes
    ...
    def find_models_to_evict(self, cache: OrderedDict, required_vram: int) -> list[str]:
        ...
        # All models with scores and VRAM sizes
        candidates = []
        ...
        # Sort by score (lowest first = first candidates)
        candidates.sort(key=operator.itemgetter(1))
        # candidates.sort(key=lambda x: x[1])

        # Greedy algorithm: Evict models until enough VRAM is free
        to_evict = []
        freed_vram = 0
        ...
        for model_tag, score, vram_size in candidates:
            ...
            # Simulate eviction and check if enough space is available now
    ...
    def evict_model(self, cache: OrderedDict, model_tag: str) -> int:
        ...
        try:
            ...
            # Clear GPU memory cache - important for actual VRAM release
