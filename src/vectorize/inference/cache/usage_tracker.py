...
class UsageTracker:
    ...
    def __init__(self, cache_file: Path) -> None:
        ...
        self.stats = defaultdict(lambda: {"count": 0, "last_access": 0})
        self._load_stats()

    def track_access(self, model_tag: str) -> None:
        """Register a model access."""
        self.stats[model_tag]["count"] += 1
        self.stats[model_tag]["last_access"] = int(time.time())

    def calculate_score(self, model_tag: str) -> float:
        """Calculate usage score for eviction decision."""
        stats = self.stats[model_tag]
        current_time = time.time()
        hours_since_access = (current_time - stats["last_access"]) / 3600
        return stats["count"] / max(hours_since_access, 0.1)
