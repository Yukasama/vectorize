...
class VRAMMonitor:
    ...
    def get_total_vram(self) -> int:
        ...
        return sys.maxsize  # CPU = unlimited (using max int)
    ...
    def estimate_model_vram(model: torch.nn.Module) -> int:
        ...
        # Buffers (BatchNorm stats, etc.)
        for buffer in model.buffers():
            total_size += buffer.numel() * buffer.element_size()

        # Overhead for gradients (even if requires_grad=False)
        # and internal PyTorch structures (~20% overhead)
